...
# 
# DESCRIPTION:
# This script is designed to get 100000 random genesets of matched gene expression level, gene length or GC content relative to the geneset of interest and calculate their internal CC and CC to the input geneset. The input files of this script are AllGene list file, the input gene list file, and the correlation matrix dataset. AllGene list needs to be ranked based on on gene expression levels, gene size or GC content. Matrix dataset are correlation coefficiencies of ranked all genes.
#
...
# set genes for the 100000 random genesets of matched expression, size or GC content (50 upper or lower in the rank for each gene in the input gene list), calculate the CC matrix
for (my $Cycle_mRand=1; $Cycle_mRand<100001; $Cycle_mRand++)	      
{	
...
#Write selected random genes into the mRand-$Cycle file 
	my $tmp ="$AllGene-mRandomGene100000-$Cycle_mRand";	
...
{
	my $tmp = "$AllGene-MatchedRandomGene100000-$Input";
	print F2OUT $tmp . "\t";
	print F3OUT $InputGene_Ranked[$Input-1] . "\t";
	for (my $Column_Numb=1; $Column_Numb<100001; $Column_Numb++)
	{
...
for (my $Cycle_Rand=1; $Cycle_Rand<100001; $Cycle_Rand++)   
{
	my $tmp ="$AllGene-mRandomGene100000-$Cycle_Rand";
