...
# set genes for the 1000 random genesets of matched size or expression (50 upper or lower in the rank for each gene in the input gene list), calculate the CC matrix
for (my $j=1; $j<201; $j++)	      
{	
	for (my $a=0; $a<$i; $a++)
	{
...
#Write selected random genes into the MatchedRand-$j file 
	my $tmp ="$AllGene-MatchedRand200-$j";	
...
for ($a=1; $a<$k; $a++)
{
	my $tmp = "$AllGene-MatchedRand200-$a";
	print F2OUT $tmp . "\t";
...
for ($a=1; $a<$TotalGeneNumb+1; $a++)
{
	my $tmp = "MatchedRand-$a";
	print F4OUT $WGM[$a-1] . "\t";
	for (my $b=1; $b<201; $b++)
	{
		print F4OUT $Matrix4[$a-1][$b-1] . "\t";
