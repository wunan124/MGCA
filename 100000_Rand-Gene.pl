...
#get gene names of the input geneset
$i=0;
my @Gene = ();
my @GenePosition = ();
...
# set genes for the 200 random genesets of matched size or expression (50 upper or lower in the rank for each gene in the input gene list), calculate the CC matrix
...
#Write selected random genes into the MatchedRand-$j file 
	my $tmp ="$AllGene-RandomGene100000-$j";	
...
for ($a=1; $a<$k; $a++)
{
	my $tmp = "$AllGene-RandomGene100000-$a";
	print F2OUT $tmp . "\t";
