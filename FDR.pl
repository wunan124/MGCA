...
#Write selected random genes into the mRand-$Cycle file 
	my $tmp ="$AllGene-mRandomGeneFDR-$Cycle_mRand";	
    ...
    while(my $line = <F2IN>)
    {
	    ...
	    $Matrix2[$Matrix_Row][$Cycle_mRand-1] = $Row [$Matrix_Column-1];
	    $Matrix_Row+=1;
	}
	close(F2IN);
...
for (my $Input=1; $Input<$TotalGeneNumb+1; $Input++)
{
	...
	{
		my $mRand_P_value = $Matrix2[$Input-1][$Column_Numb-1];
		my $Threshold = 0.001;
        if($mRand_P_value < $Threshold)
        {
            $Count+=1;
        }	
	}
